...
SONGS_DIR = os.path.join(PROJECT_ROOT, "songs/Arijit Singh")  # folder for Arijit Singh songs

root = Tk()
root.title("Arijit Singh Songs")
...
def fetch_songs():
    ...
    cursor.execute("SELECT song_id, song_path FROM arijit_songs")
...
def get_song_path(song_name):
    ...
    cursor = connection.cursor()
    cursor.execute("SELECT song_path FROM arijit_songs WHERE song_path LIKE %s", (f"%{song_name}%",))
    song = cursor.fetchone()
    connection.close()
    if song:
        # Convert stored path to OS path
        return os.path.join(PROJECT_ROOT, song[0].replace("/", os.sep))
    return None
