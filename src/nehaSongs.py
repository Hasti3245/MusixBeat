...
SONGS_DIR = os.path.join(PROJECT_ROOT, "songs/Neha Kakkar")  # folder for Neha Kakkar songs

# ---------------- Tkinter Setup ----------------
root = Tk()
root.title("Neha Kakkar Songs")
...
def fetch_songs():
    ...
    cursor.execute("SELECT song_id, song_path FROM neha_songs")
...
def get_song_path(song_name):
    ...
    cursor = connection.cursor()
    cursor.execute("SELECT song_path FROM neha_songs WHERE song_path LIKE %s", (f"%{song_name}%",))
