...
SONGS_DIR = os.path.join(PROJECT_ROOT, "songs/K.K.")  # example album
...
def fetch_songs():
    ...
    cursor.execute("SELECT song_id, song_path FROM kk_songs")
...
def get_song_path(song_name):
    connection = mysql.connector.connect(host="localhost", user="root", password="", database="music_player")
    cursor = connection.cursor()
    cursor.execute("SELECT song_path FROM kk_songs WHERE song_path LIKE %s", (f"%{song_name}%",))
