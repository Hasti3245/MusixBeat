...
ASSETS_DIR = os.path.join(PROJECT_ROOT, "assets")  # folder for images/icons
...
def login():
    ...
    if user:
        ...
        subprocess.run(["python", demo_path])
        root.quit()
    else:
...
tk.Label(login_frame, text="User Login", font=("Arial", 30, "bold"), bg="#f0f0f0", fg="#2E3B4E").grid(row=0, column=0, columnspan=2, pady=20)
...
tk.Label(signup_frame, text="Phone Number:", font=("Arial", 12, "bold"), bg="#f0f0f0", fg="#2E3B4E").grid(row=4, column=0, pady=15, padx=25, sticky="e")
entry_signup_phone = tk.Entry(signup_frame, font=entry_font, width=25)
entry_signup_phone.grid(row=4, column=1, pady=15, padx=25)
